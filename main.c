...
void editor_update_syntax(erow *row) {
	...
	while (i < row->rsize){
		...
		if (mcs_len && mce_len && !in_string) {
			if (in_comment) {
				row->hl[i]=HL_MLCOMMENT; //comment text 
				if (!strncmp(&row->render[i],mce,mce_len)) { //comment end
			...
			} else if (!strncmp(&row->render[i],mcs,mcs_len)) { //comment start
		...
		if (prev_sep) { //check if the current character is a keyword
			int j;
...
int editor_row_rx_to_cx(erow *row,int rx) {
	...
	for (cx=0;cx<row->size;cx++){
		if (row->chars[cx]=='\t') {
			cur_rx += (YED_TAB_STOP-1)-(cur_rx%YED_TAB_STOP);
		}
		cur_rx++;
...
void editor_save() {
	if (E.filename==NULL) { //open new file
		E.filename = editor_prompt("save as : %s (ESC to cancel)",NULL);
		if (E.filename==NULL) {
			editor_set_status_message("save aborted");
			return;
		}
		editor_select_syntax_highlight();//when save as specific file,select the syntax highlight style
	}
...
void editor_find() {
	...
	char *query = editor_prompt("Search: %s (ESC to cancel, Arrow keys find next, Enter to quit)",editor_find_callback);
...
void editor_draw_statusbar(struct abuf *ab) {
	...
	E.syntax? E.syntax->file_type: "no filetype",E.cy+1,E.numrows);
...
char *editor_prompt(char *prompt,void (*callback)(char *,int)) {
	...
	while (1) {
		...
		} else if (c=='\x1b') { //cancel input with press ESC
		...
		} else if (!iscntrl(c) && c < 128) { //input normal character
...
void editor_process_keypress() {
	...
	switch (c) {
		...
		case CTRL_KEY('h'):
		case DEL_KEY:
			if (c==DEL_KEY) { //delete right of the cursor character
